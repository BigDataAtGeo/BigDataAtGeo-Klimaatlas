<template>
  <div id="content">
    ...
    <vueper-slides :touchable=false :arrows=false class="no-shadow" fixedHeight="17rem"
      ...
      <vueper-slide v-for="(cell,index) in selectedCells" :key="cell.properties.id"
      ...
      <template v-slot:bullet="{active, slideIndex}">
        <b-icon icon="square-fill"
                :style="[ active ? 'width: 18px; height: 18px;' : 'width: 9px; height: 9px;']+'color:'+ generateColor(selectedCells[slideIndex].properties.id,0)+';'"></b-icon>
      </template>
    </vueper-slides>
  </div>

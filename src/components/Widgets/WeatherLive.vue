        ...
        methods:{
            ...
            loadWeather(){
                if(this.selectedCell!=null){
                    axios.get('https://api.openweathermap.org/data/2.5/weather?lat='+this.selectedCell.geometry.coordinates[1]+'&lon='+
                    this.selectedCell.geometry.coordinates[0]+'&units=metric&lang=de&appid=e1281e3f9e6ce6ea4db955a47f9f2fca&lang=de')
                .then(response=>this.weather=response.data)
                .catch((error) => console.error("fetch data error: failed to load JSON from server", error));
                }
                else{axios.get('https://api.openweathermap.org/data/2.5/weather?zip=97074,de&units=metric&lang=de&appid=e1281e3f9e6ce6ea4db955a47f9f2fca&lang=de')
                .then(response=>this.weather=response.data)
                .catch((error) => console.error("fetch data error: failed to load JSON from server", error));}
            }
        }

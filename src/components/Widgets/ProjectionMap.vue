<template>
  <l-map id="leaflet" :options="mapOptions" v-bind:class="{ blurry: isLoading }" :zoom="mapOptions.zoom"
    ...
    <l-layer-group layerType="overlay">
      <div v-if="geojson">
        <l-polygon v-for="polygon,index of this.polygons" :lat-lngs="polygon" :color="generateColor(ids[index], 0)"
                   :interactive="booleanF" :bubblingMouseEvents="booleanF"
